    ...
    return (
      <div className="max-w-7xl mx-auto p-6 space-y-6 animate-pulse">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          ...
          <div className="md:col-span-1 h-64 bg-slate-200 rounded-2xl"></div>
        </div>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-50">
        <div className="p-8 bg-white border border-red-100 shadow-xl rounded-2xl text-center max-w-md">
      ...
      <div className="p-4">
        <div className="grid grid-cols-5 gap-2">
          {questions.map((q, index) => {
            ...
            if (isActive) {
              buttonClass += 'bg-indigo-600 text-white shadow-lg shadow-indigo-200 ring-2 ring-indigo-600 ring-offset-2';
            } else if (isDoubtful) {
              buttonClass += 'bg-amber-100 text-amber-700 border border-amber-300 hover:bg-amber-200';
            } else if (isAnswered) {
              buttonClass += 'bg-emerald-100 text-emerald-700 border border-emerald-300 hover:bg-emerald-200';
            } else {
              buttonClass += 'bg-white text-slate-600 border border-slate-200 hover:border-slate-400 hover:bg-slate-50';
            }
        ...
        <div className="mt-6 space-y-2 text-xs text-slate-500 border-t border-slate-100 pt-4">
            <div className="flex items-center gap-2">
             ...
             <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-amber-100 border border-amber-300 rounded"></div> Ragu-ragu
            </div>
             <div className="flex items-center gap-2">
                <div className="w-3 h-3 bg-white border border-slate-200 rounded"></div> Belum Dijawab
            </div>
        </div>
      </div>
    </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[500px] flex flex-col">
              {currentQuestion ? (
                <>
                  <div className="p-6 md:p-8 flex-1">
                    <div className="flex justify-between items-start mb-6">
                        <span className="inline-flex items-center justify-center px-3 py-1 rounded-full bg-slate-100 text-slate-600 text-xs font-bold uppercase tracking-wider">
                  ...
                  <div className="bg-slate-50 p-4 md:p-6 border-t border-slate-200 flex flex-col-reverse md:flex-row justify-between items-center gap-4">
                     <button
                        ...
                        className={`text-sm font-medium flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${
                            !answers[currentQuestion.id] ? 'opacity-0 pointer-events-none' : 'text-red-600 hover:bg-red-50'
                        }`}
                      >
                        <Icons.Trash />
                        Hapus Jawaban
                      ...
                      <div className="flex w-full md:w-auto gap-3">
                        <button
                            ...
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-5 py-2.5 text-sm font-semibold text-slate-700 bg-white border border-slate-300 rounded-xl hover:bg-slate-50 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                        >
                            <Icons.ChevronLeft />
                            Sebelumnya
                        </button>
                        ...
                        {currentQuestionIndex === questions.length - 1 ? (
                            ...
                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-6 py-2.5 text-sm font-semibold text-white bg-indigo-600 rounded-xl hover:bg-indigo-700 active:scale-95 transition-all shadow-md shadow-indigo-100"
                            >
                            Selanjutnya
                            <Icons.ChevronRight />
                            </button>
                ...
                <div className="flex-1 flex flex-col items-center justify-center text-center p-10">
                   ...
                   <h2 className="text-xl font-semibold text-slate-800">Tidak ada soal dimuat.</h2>
                   <p className="text-slate-500 mt-2">Mungkin ada masalah dengan konfigurasi ujian ini.</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute right-0 top-0 h-full w-80 bg-white shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col">
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
                ...
                <button onClick={() => setIsSidebarVisible(false)} className="p-2 bg-white rounded-full text-slate-500 hover:text-slate-800 shadow-sm border border-slate-200">
                    <Icons.ChevronRight />
                </button>
            </div>
