            ...
            <td className="px-6 py-4 whitespace-nowrap align-top">
                ...
                <div className="mt-4 ml-14">
                    <button
                        ...
                        className={`flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg shadow-sm transition-all ${
                            hasChanges
                ...
                <div className="flex flex-wrap gap-2">
                    {allClasses.map(cls => (
                        <label key={cls.id} className={`select-none flex items-center p-2 rounded-lg cursor-pointer border transition-all text-xs ${assignedClasses.includes(cls.id) ? 'bg-emerald-50 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-700 text-emerald-800 dark:text-emerald-300' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400'}`}>
                            ...
                            />
                            <div className={`w-3 h-3 rounded-sm border mr-2 flex items-center justify-center ${assignedClasses.includes(cls.id) ? 'bg-emerald-500 border-emerald-500' : 'border-slate-300 dark:border-slate-500'}`}>
                                {assignedClasses.includes(cls.id) && <svg className="w-2 h-2 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
                            </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {allSubjects.map(sbj => (
                        <label key={sbj.id} className={`select-none flex items-center p-2 rounded-lg cursor-pointer border transition-all text-xs ${assignedSubjects.includes(sbj.id) ? 'bg-blue-50 dark:bg-blue-900/30 border-blue-200 dark:border-blue-700 text-blue-800 dark:text-blue-300' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700 text-slate-600 dark:text-slate-400'}`}>
                            ...
                            />
                            <div className={`w-3 h-3 rounded-sm border mr-2 flex items-center justify-center ${assignedSubjects.includes(sbj.id) ? 'bg-blue-500 border-blue-500' : 'border-slate-300 dark:border-slate-500'}`}>
                                {assignedSubjects.includes(sbj.id) && <svg className="w-2 h-2 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M5 13l4 4L19 7" /></svg>}
                            </div>
                            <span className="font-medium">{sbj.name}</span>
                        </label>
                    ))}
