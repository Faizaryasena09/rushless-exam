...
export async function POST(request) {
    ...
    try {
        ...
        const result = await transaction(async (query) => {
            ...
            // 6.5 Verify SEB Headers if required
            if (settings.require_seb) {
                ...
                if (!sebConfigKeyHeader && !sebRequestHashHeader) {
                    throw new Error('Safe Exam Browser headers missing. Please use SEB to start the exam.');
                }
                if (settings.seb_config_key && settings.seb_config_key.trim() !== '') {
                    if (sebConfigKeyHeader !== settings.seb_config_key) {
                        throw new Error('SEB Configuration Key mismatch. Please ensure you are using the correct SEB config file.');
                    }
                }
            }
